...
class task_safebet(W_Gym):
    ...
    def _step(self, action, guess = -1, safebet = 0):
            R_ext = 0
            R_int = 0
    ...
    def step(self, action, guess= -1, safebet0 = 0, safebet1 = 0, safebet2 = 0):
        reward_E = 0
        reward_I = 0
        # advance time
        self.tot_t += self.dt
        ...
        if not is_error and hasattr(self, '_step'):    
            tR_ext, tR_int = self._step(action, guess, safebet0)
            reward_E += tR_ext
            rguess = tR_int      
