...
plt.plot(xs, gp.(['GPav_', vn]){1}, gp.(['GPste_', vn]){1}, 'shade', 'color', cols);
plt.setfig_ax('xlim', xlm, 'legend', arrayfun(@(x)sprintf("pT = %.0f, pA = %.0f", gp.GPav_gpT{1}(x), gp.GPav_gpA{1}(x)),1:length(gp.GPav_gpA{1})));
plt.update('pcorrect', ' ');
%%
plt.figure;
...
plt.setfig_ax('legend', plt.custom_vars.leg_MBMF, 'legloc', 'SE', ...
    'xlabel', namecond, 'ylabel', 'log likelihood', 'xlim',[0.5, 5.5], ...
...
plt.setfig_ax('legend', plt.custom_vars.leg_MBMF, 'legloc', 'SW', ...
    'xlabel', namecond, 'ylabel', 'weight (MB)', 'xlim',[0.5, 5.5], ...
    'xtick', 1:5, 'xticklabel', W.iif(cond == 'ambiguity', gp.GPav_gpA{1}', gp.GPav_gpT{1}'));
plt.update('MLEratio','  ');
